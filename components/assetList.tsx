...
import { AssetTools } from "@/typings";
import {
  CalendarDaysIcon,
  CubeTransparentIcon,
  PlayCircleIcon,
...
function AssetList({ asset }: Props) {
  const colors = [
    "text-yellow-500",
    "text-red-500",
    "text-blue-500",
  ...
  return (
    <div id="assets" className="w-full lg:h-screen p-2 px-4">
              ...
              <div key={assets?._id} className="max-w-sm min-w-[350px]  ">
                <div className="items-center shadow-sm shadow-gray-400 rounded-xl group cursor-pointer   ">
                  ...
                  <div className="hidden group-hover:block relative top-5 bottom-5 py-5 px-5 ">
                    <p
                      className={`relative top-0 py-2 bg-${assets?.tagBgColor}-500 text-gray-800 font-bold  rounded-br-lg rounded-tl-lg`}
                    >
                      <span className="ml-5"> {assets?.assetTag}</span>
                    </p>
                    <h1
                      className={`mb-3 mt-4 text-2xl font-bold text-${assets?.titelColor}-500`}
                    >
                      {assets?.title}
                    </h1>
                      ...
                      <div className="flex items-center">
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="font-mono">
                            {new Date(assets?._createdAt).toLocaleDateString(
                              "en-US",
                              {
                                day: "numeric",
                                month: "long",
                    ...
                    <ClientSideRoute route={`/asset/${assets?.slug.current}`}>
                      <button
                        className="flex items-center justify-center right-0 p-2 mt-4 mb-4 text-sm font-bold w-full text-white bg-[#005eff] py-2 
                rounded-lg shadow-lg hover:bg-[#3982ff] hover:scale-105 ease-in duration-300"
                      >
