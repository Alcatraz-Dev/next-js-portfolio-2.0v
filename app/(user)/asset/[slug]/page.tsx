...
export async function generateStaticParams() {
  const quary = groq`*[_type =='asset']{
          slug
      }`;
  const slugs: AssetTools[] = await client.fetch(quary);
  const slagRoutes = slugs?.map((slug) => slug.slug.current);
  return slagRoutes?.map((slug) => ({
    slug,
  }));
}
async function Asset({ params: { slug } }: Props) {
  const query = groq`
  ...
  const asset: AssetTools = await client.fetch(query, { slug });
  ...
  return (
    <div className="w-full h-full">
      <div className="w-screen h-[50vh] relative">
        ...
        >
          <h2
            className={`py-1  text-4xl  font-bold text-${asset?.titelColor}-500`}
          >
            {asset?.title}
          </h2>
          ...
          <div className="flex items-center space-x-4 mt-10">
            <div key={asset?._id} className="flex items-center">
              <div className="flex items-center space-x-1">
                <Image
                  src={urlFor(asset?.author?.mainImage)
                    .width(500)
                    .height(500)
                    .url()}
                ...
                <span className="font-mono">
                  {asset?.author?.name}{" "}
                  <p>
                    {new Date(asset?._createdAt).toLocaleDateString("en-US", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })}
      ...
      <div className="  mx-5">
        ...
        <div className=" flex  items-center justify-center   space-x-3 mt-5 mb-5   ">
          {asset?.customButton?.map((buttons) => (
            <div
              key={buttons?._id}
        ...
        <div className=" flex items-center justify-center space-x-3 mt-5 mb-5">
          {asset?.assetFile?.map((assets) => (
            <div
              key={assets?._id}
              className="flex items-center justify-center "
            >
              <a
                href={`https://cdn.sanity.io/files/g8tq43y4/cms/${assets?.fileId}?dl`}
                download
              >
                <div
                  className={`flex items-center justify-center right-0 p-3 py-3 mt-4 mb-4 text-xs font-bold w-auto h-auto text-white bg-${assets?.buttonBgColor}-600 py-2 
              rounded-lg shadow-lg hover:bg-${assets?.buttonBgColor}-300 hover:scale-105 ease-in duration-300`}
                >
                  <span className="mr-3">{assets?.name}</span>
                  <Image
                    src={urlFor(asset?.mainImage).url()}
                    alt={assets?.name}
                    width={20}
